	...
	public void mergePlayerData(Connection conn, String oldUID, String uid) throws SQLException, ProcessingException {
		...
		try {
			...
			while (rs.next()) {
				...
				updatePS = conn.prepareStatement("UPDATE PLAYER_DATA SET KILLS=KILLS+?, DEATHS=DEATHS+?, MECHANICS=MECHANICS+?, GOALS=GOALS+?, PASSES=PASSES+?, INTERCEPTIONS=INTERCEPTIONS+?, CAPTURES=CAPTURES+?, ROUNDS=ROUNDS+? WHERE UID=? AND SID=?");
				...
				updatePS.setInt(10, sid);
				int updated = updatePS.executeUpdate();
				
				if (updated == 0) {
					ResourceManager.releaseResources(updatePS);
					updatePS = conn.prepareStatement("INSERT INTO PLAYER_DATA (UID,KILLS,DEATHS,MECHANICS,GOALS,PASSES,INTERCEPTIONS,CAPTURES,ROUNDS,LAST_SEEN,SID) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
					...
					updatePS.setInt(11, sid);
					updated = updatePS.executeUpdate();
					if (updated != 1) {
						throw new ProcessingException("Did not insert new entry for " + uid + " sid=" + sid);
					}
				}
				
				ResourceManager.releaseResources(updatePS);
				
				updatePS = conn.prepareStatement("DELETE FROM PLAYER_DATA WHERE UID=? AND SID=?");
				updatePS.setString(1, oldUID);
				updatePS.setInt(2, sid);
	...
	public void mergeWeeklyData(Connection conn, String oldUID, String uid) throws SQLException, ProcessingException {
		...
		try {
			...
			while (rs.next()) {
				...
				updatePS = conn.prepareStatement("SELECT WEAPON_KILLS, KILLED_BY FROM WEEKLY_DATA WHERE UID=? AND SID=? AND WEEK_DATE=? AND GAME_MODE=? AND ELO=? AND PLATFORM=?");
				updatePS.setString(1, uid);
				updatePS.setInt(2, sid);
				...
				if (rsUpdate.next()) {
					...
					ResourceManager.releaseResources(updatePS);
					updatePS = conn.prepareStatement("UPDATE WEEKLY_DATA SET KILLS=KILLS+?, DEATHS=DEATHS+?, MECHANICS=MECHANICS+?, CAPTURES=CAPTURES+?, ROUNDS=ROUNDS+?, WEAPON_KILLS=?, KILLED_BY=?, TOTAL_TIME=TOTAL_TIME+? WHERE UID=? AND SID=? AND WEEK_DATE=? AND GAME_MODE=? AND ELO=? AND PLATFORM=?");
					updatePS.setInt(1, kills);
					updatePS.setInt(2, deaths);
					updatePS.setInt(3, mechanics);
					...
					updatePS.setString(9, uid);
					updatePS.setInt(10, sid);
					updatePS.setObject(11, weekDate);
					updatePS.setInt(12, gameMode);
					updatePS.setInt(13, elo);
					updatePS.setInt(14, platform);
					if (updatePS.executeUpdate() == 0) {
						throw new ProcessingException("Did not combine weekly row for " + uid + " for sid=" + sid + ", week=" + weekDate + ", game=" + gameMode + ", elo=" + elo);
					}
					
					ResourceManager.releaseResources(updatePS);
					updatePS = conn.prepareStatement("DELETE FROM WEEKLY_DATA WHERE UID=? AND SID=? AND WEEK_DATE=? AND GAME_MODE=? AND ELO=? AND PLATFORM=?");
					updatePS.setString(1, oldUID);
					updatePS.setInt(2, sid);
					updatePS.setObject(3, weekDate);
					updatePS.setInt(4, gameMode);
					...
					updatePS.setInt(6, platform);
					if (updatePS.executeUpdate() == 0) {
						throw new ProcessingException("Did not delete weekly row for " + oldUID + " for sid=" + sid + ", week=" + weekDate + ", game=" + gameMode + ", elo=" + elo);
					}
				} else {
					...
					ResourceManager.releaseResources(updatePS);
					updatePS = conn.prepareStatement("UPDATE WEEKLY_DATA SET UID=? WHERE UID=? AND SID=? AND WEEK_DATE=? AND GAME_MODE=? AND ELO=? AND PLATFORM=?");
					updatePS.setString(1, uid);
					updatePS.setString(2, oldUID);
					updatePS.setInt(3, sid);
					updatePS.setObject(4, weekDate);
					updatePS.setInt(5, gameMode);
					updatePS.setInt(6, elo);
					updatePS.setInt(7, platform);
					if (updatePS.executeUpdate() == 0) {
						throw new ProcessingException("Did not update weekly row for " + oldUID + " to " + uid + " for sid=" + sid + ", week=" + weekDate + ", game=" + gameMode + ", elo=" + elo);
					}
				}
				...
				updatePS = conn.prepareStatement("SELECT UID,SID,WEEK_DATE,GAME_MODE,ELO,PLATFORM,KILLED_BY FROM WEEKLY_DATA WHERE KILLED_BY LIKE ?");
				updatePS.setString(1, "%\"" + oldUID + "\"%");
				rsUpdate = updatePS.executeQuery();
				while (rsUpdate.next()) {
					String weekUID = rsUpdate.getString("UID");
					int weekSID = rsUpdate.getInt("SID");
					...
					updatePSTwo = conn.prepareStatement("UPDATE WEEKLY_DATA SET KILLED_BY = ? WHERE UID=? AND SID=? AND WEEK_DATE=? AND GAME_MODE = ? AND ELO = ? AND PLATFORM=?");
					updatePSTwo.setString(1, DBUtil.createDBMap(killedBy));
					updatePSTwo.setString(2, weekUID);
					updatePSTwo.setInt(3, weekSID);
					updatePSTwo.setObject(4, weekWeekDate);
					updatePSTwo.setInt(5, weekGameMode);
					updatePSTwo.setInt(6, weekElo);
					updatePSTwo.setInt(7, uPlatform);
					if (updatePSTwo.executeUpdate() == 0) {
						throw new ProcessingException("Did not update weekly reference row for " + weekUID + " for sid=" + weekSID + ", week=" + weekWeekDate + ", game=" + weekGameMode + ", elo=" + weekElo);
					}
